<%-- 
    Document   : my-aggregate-files
    Created on : Oct 28, 2011, 2:10:31 PM
    Author     : bullard
--%>
...
<script type="text/javascript">
    ...
    if (!$$('#subtabs-right ul')[0] || !$$('#subtabs-right ul')[0].hasChildNodes() || ($$('#subtabs-right ul li')[0] && $$('#subtabs-right ul li')[0].id != "my-microstructure-files")) {
        ...
        urls[4] = "<%=request.getContextPath()%>/lmy-files/initializeMyTransportFiles.do";
...
<html:form action="/my-files/editMyAggregateFiles.do">
	<bean:size id="aggregateOperationsNumber" name="myAggregateFilesForm" property="aggregateOperations" />
	<logic:greaterThan name="aggregateOperationsNumber" value="0">
		<fieldset id="aggregate-operations">
			...
			<nested:iterate name="myAggregateFilesForm" property="aggregateOperations" indexId="ind">
						...
						<div class="collapsable-content expanded">
								...
								<nested:notEmpty property="filesList">
									<nested:iterate property="filesList" indexId="i">
										...
										<tr class="odd-file">
											...
											<td>
												<a href="javascript:viewAggregateFile('aggregateOperations(<%=operationName%>).filesList[<%=i%>]','show-aggregate-file-content_<%=ind%>_<%=i%>','to-reload');" id="show-aggregate-file-content_<%=ind%>_<%=i%>">Show</a>
												<nested:link action="/my-files/downloadAggregateFile.do" property="linkParameters">Export</nested:link>
												...
												<nested:equal property="viewContent" value="true">
													...
													<nested:equal property="type" value="image">
														...
														<div id="aggregate-file-content_<%=ind%>_<%=i%>" class="to-reload file-image"><nested:link action="/my-files/viewImage.do" target="_blank" property="linkParameters"><img src="/vcctl/image/<%=sliceName%>" /></nested:link></div>
													</nested:equal>
												</nested:equal>
							...
							<nested:notEmpty property="filesList">
								...
								<logic:greaterThan name="filesNumber" value="0">
									Export all files for this operation in one zip file: <nested:link action="/my-files/downloadAggregateFile.do" paramId="operation" paramProperty="name" styleClass="link"><nested:write property="zipArchiveName" /></nested:link>
								</logic:greaterThan>
							</nested:notEmpty>
						</div>
					</fieldset>
