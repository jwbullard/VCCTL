...
class DerbyCorrelationExporter:
    ...
    def export_cement_correlation_files(self):
        ...
        try:
            ...
            # Get all cement materials with their correlation data
            query = """
                SELECT name, c3acorr, c4fcorr, silcorr, alucorr, c3scorr
                FROM cement 
                ORDER BY name
            """
            ...
            logger.info(f"Found {len(results)} cement records")
            
            # Track statistics
            stats = {
                'total_cements': len(results),
                'c3a_files': 0,
                'c4f_files': 0,
                'sil_files': 0,
                'alu_files': 0,
            ...
            for row in results:
                name, c3acorr, c4fcorr, silcorr, alucorr, c3scorr = row
                logger.info(f"Processing cement: {name}")
                
                # Export each correlation file type
                if c3acorr is not None:
                    try:
                        # Convert binary data to bytes if needed
                        if isinstance(c3acorr, str):
                            # If it's a hex string, decode it
                            if c3acorr.startswith('\\x') or len(c3acorr) % 2 == 0:
                                try:
                                    c3a_bytes = bytes.fromhex(c3acorr.replace('\\x', ''))
                                except ValueError:
                                    c3a_bytes = c3acorr.encode('utf-8')
                            else:
                                c3a_bytes = c3acorr.encode('utf-8')
                        else:
                            c3a_bytes = bytes(c3acorr)
                ...
                if c4fcorr is not None:
                    try:
                        # Convert binary data to bytes if needed
                        if isinstance(c4fcorr, str):
                            if c4fcorr.startswith('\\x') or len(c4fcorr) % 2 == 0:
                                try:
                                    c4f_bytes = bytes.fromhex(c4fcorr.replace('\\x', ''))
                                except ValueError:
                                    c4f_bytes = c4fcorr.encode('utf-8')
                            else:
                                c4f_bytes = c4fcorr.encode('utf-8')
                        else:
                            c4f_bytes = bytes(c4fcorr)
                ...
                for corr_data, extension, stat_key in [
                    (silcorr, 'sil', 'sil_files'),
                    (alucorr, 'alu', 'alu_files'),
                    (c3scorr, 'c3s', 'c3s_files')
                ]:
                    if corr_data is not None:
                        try:
                            if isinstance(corr_data, str):
                                if corr_data.startswith('\\x') or len(corr_data) % 2 == 0:
                                    try:
                                        corr_bytes = bytes.fromhex(corr_data.replace('\\x', ''))
                                    except ValueError:
                                        corr_bytes = corr_data.encode('utf-8')
                                else:
                                    corr_bytes = corr_data.encode('utf-8')
                            else:
                                corr_bytes = bytes(corr_data)
...
def main():
    ...
    derby_path = "/Users/jwbullard/Library/CloudStorage/OneDrive-TexasA&MUniversity/Documents/Projects/Modeling/VCCTL-THAMES-SPRING/vcctl-gtk/VCCTL"
