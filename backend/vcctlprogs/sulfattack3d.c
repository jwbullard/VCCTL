...
int main(void)
{
    ...
    ***/
	...
	for (iz = 1; iz < Zsyssize + 1; iz++) {
		for (iy = 0; iy < Ysyssize; iy++) {
			for (ix = 0; ix < Xsyssize; ix++) {
				...
				if (inval == BRUCITE) Bruciteorig[iz]++;
	...
	fclose(micfile);	
	printf("Initial counts for CH, AFM, C3AH6 and ettringite(2) are %ld, %ld, %ld, %ld, and %ld.\n",chinit,afminit,c3ah6init,ettrinit,ettrc4init);
	fflush(stdout);
			...
			***/
			...
			if (ich != 0) {
				...
				if ((phid != POROSITY) && (phid != AFMC)
					&& (phid != CSH) && (phid != CH)
					&& (phid != AFM) && (phid != EMPTYP)
					&& (phid != DRIEDP) && (phid != C3AH6)
					&& (phid!=POZZCSH) && (phid!=SLAGCSH)) {
						ich=0;
				}
...
void allmem(void)
{
	...
	if (!Mic || !React || !Ndiff || !Nrch || !Nrafm || !Nrc3ah6
		|| !Afmorig || !Noch || !Ettrorig || !Bruciteorig
		|| !Gypsumorig || !C3ah6orig || !Ettrc4aforig
		|| !Chorig || !Nrafmc || !Afmcorig || !Ccorig
		|| !Nrcap || !Nrgel || !Straingyp || !Strainbrucite
		|| Strainettr || Strainafm || Xnew || Ynew || Znew) {
...
void freeallmem(void)
{
	...
	if (Ndiff) free_livector(Ndiff);
	if (Nrch) free_livector(Nrch);
	if (Nrafm) free_livector(Nrafm);
	if (Nrc3ah6) free_livector(Nrc3ah6);
	if (Afmorig) free_livector(Afmorig);
	if (Noch) free_livector(Noch);
	if (Ettrorig) free_livector(Ettrorig);
	if (Bruciteorig) free_livector(Bruciteorig);
	if (Gypsumorig) free_livector(Gypsumorig);
	if (C3ah6orig) free_livector(C3ah6orig);
	if (Ettrc4aforig) free_livector(Ettrc4aforig);
	if (Chorig) free_livector(Chorig);
	if (Nrafmc) 	free_livector(Nrafmc);
	if (Afmcorig) free_livector(Afmcorig);
	if (Ccorig) free_livector(Ccorig);
	if (Nrcap) free_livector(Nrcap);
	if (Nrgel) free_dvector(Nrgel);
	if (Straingyp) free_fvector(Straingyp);
	if (Strainbrucite) free_fvector(Strainbrucite);
	if (Strainettr) free_fvector(Strainettr);
	if (Strainafm) free_fvector(Strainafm);
