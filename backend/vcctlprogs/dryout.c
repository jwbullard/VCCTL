...
int main(void)
{
    ...
    ***/
    ...
    for (k = 0; k < Zsyssize; k++) {
        for (j = 0; j < Ysyssize; j++) {
            for (i = 0; i < Xsyssize; i++) {
                ...
                Mic[i][j][k] = valout;
                if (valout == INERTAGG) aggcount++;
                if (valout == CSH || valout == POZZCSH) cshcount++;
                if (valout == POROSITY) satporecount++;
                if (valout == EMPTYP || valout == EMPTYDP || valout == DRIEDP) dryporecount++;
            }
        }
    }
    ...
    /***
    * Calculate current degree of saturation and minimum possible degree of saturation
    ***/

    vfcsh = ((float)cshcount)/((float)((Xsyssize * Ysyssize * Zsyssize) - aggcount));
    gelporosity = vfcsh * Gelporefrac;
    satcap = ((float)satporecount)/((float)((Xsyssize * Ysyssize * Zsyssize) - aggcount));
    drycap = ((float)dryporecount)/((float)((Xsyssize * Ysyssize * Zsyssize) - aggcount));
    ...
    /***
    * How many pixels of saturated porosity should we remove to achieve this degree of saturation?
    ***/

    target_satcap = (target_deg_sat * (satcap + gelporosity + drycap)) - gelporosity;
    target_satporecount = (int)((target_satcap * ((float)((Xsyssize * Ysyssize * Zsyssize) - aggcount))) + 0.5);
