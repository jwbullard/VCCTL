...
void harm(double theta, double phi) {
  ...
  p[7][0] =
      0.0625 * x * (429. * (pow(x, 6))-693. * (pow(x, 4)) + 315. * x * x - 35.);
  p[7][1] = 0.0625 * 7. * s *
            (429. * (pow(x, 6))-495. * (pow(x, 4)) + 135. * x * x - 5.);
  p[7][2] =
      0.125 * 63. * x * (1. - x * x) * (143. * (pow(x, 4))-110. * x * x + 15.);
    ...
    if (n == 0) {
      ...
      for (m = n; m >= -n; m--) {
        if (m >= 0) {
          ...
          realnum = (pow(-1., m))*sqrt(((2 * n + 1) / 4. / Pi) * fac(n - m) /
                                       fac(n + m)) *
                    p[n][m];
  ...
  for (n = 9; n <= Nnn; n++) {
    ...
    realnum =
        (pow(-1., n))*sqrt(((2 * n + 1) / 4. / Pi) * fac(n - n) / fac(n + n)) *
        p[n][n];
