                ...
                ***/
                ...
                if ((dist - 0.5) <= ((float)irad)) {
                    ...
                    if (wflg == Place) {
                        ...
                        && (Cemreal.val[getInt3dindex(Cemreal,xp,yp,zp)] != POROSITY)) {
                        /* or check placement */
                        nofits = 1;
                    }
                }
        ...
        while (k <= nzp) {
            ...
            while (j <= nyp) {
                ...
                while (i <= nxp) {
                    ...
                    k1 = zin + k;
                    k1 += checkbc(k1,Zsyssize);
                    if (Bbox.val[getInt3dindex(Bbox,i,j,k)] != POROSITY && Bbox.val[getInt3dindex(Bbox,i,j,k)] < FCHECK) {
...
int adjustvol(int diff, int nxp, int nyp, int nzp)
{
    ...
    if (diff > 0) {
        ...
        for (i = 2; i < nxp - 1; i++) {
            for (j = 2; j < nyp - 1; j++) {
                for (k = 2; k < nzp - 1; k++) {
                    if (Bbox.val[getInt3dindex(Bbox,i,j,k)] == POROSITY &&
                        ( (Bbox.val[getInt3dindex(Bbox,i+1,j,k)] == C3S) || (Bbox.val[getInt3dindex(Bbox,i-1,j,k)] == C3S)
                          || (Bbox.val[getInt3dindex(Bbox,i,j+1,k)] == C3S) || (Bbox.val[getInt3dindex(Bbox,i,j-1,k)] == C3S)
                          || (Bbox.val[getInt3dindex(Bbox,i,j,k+1)] == C3S) || (Bbox.val[getInt3dindex(Bbox,i,j,k-1)] == C3S)
                        ) ) {
            ...
            for (j = 1; j <= nyp - 1; j++) {
                for (k = 1; k <= nzp - 1; k++) {
                    if (Bbox.val[getInt3dindex(Bbox,i,j,k)] == C3S &&
                        ( (Bbox.val[getInt3dindex(Bbox,i+1,j,k)] == POROSITY) || (Bbox.val[getInt3dindex(Bbox,i-1,j,k)] == POROSITY)
                          || (Bbox.val[getInt3dindex(Bbox,i,j+1,k)] == POROSITY) || (Bbox.val[getInt3dindex(Bbox,i,j-1,k)] == POROSITY)
                          || (Bbox.val[getInt3dindex(Bbox,i,j,k+1)] == POROSITY) || (Bbox.val[getInt3dindex(Bbox,i,j,k-1)] == POROSITY)
                        ) ) {
                        /* add i,j,k to surface pixels */
                        sp[numsp].x = i;
                        sp[numsp].y = j;
    ...
    for (n = 1; n <= absdiff; n++) {
        ...
        ***/
        ...
        if (Bbox.val[getInt3dindex(Bbox,sp[choice].x,sp[choice].y,sp[choice].z)] == C3S) {
            Bbox.val[getInt3dindex(Bbox,sp[choice].x,sp[choice].y,sp[choice].z)] = POROSITY;
            count--;
        } else {
            Bbox.val[getInt3dindex(Bbox,sp[choice].x,sp[choice].y,sp[choice].z)] = C3S;
            count++;
        }
...
void striplayer(int nxp, int nyp, int nzp)
{
    ...
    for (k = 1; k < nzp; k++) {
        for (j = 1; j < nyp; j++) {
            for (i = 1; i < nxp; i++) {
                if (Bbox.val[getInt3dindex(Bbox,i,j,k)] > FCHECK) Bbox.val[getInt3dindex(Bbox,i,j,k)] = POROSITY;
            }
        }
    }
                        ...
                        ***/
                        ...
                        if (( (testgyp > Probgyp)
                            ...
                            if ((fabs(Probanh - 1.0) < TINY)
                                ...
                                N_hemi += vol;
                                nump = checkpart(x,y,z,nnxp,nnyp,nnzp,vol,Npart+1,HEMIHYD,Place);
                                numpartplaced++;
                                Particle[Npart]->partphase = HEMIHYD;

                            } else {

                                nump=checkpart(x,y,z,nnxp,nnyp,nnzp,vol,Npart+1,GYPSUM,Place);
                                numpartplaced++;
...
int checkfloc(struct particle *partpoint, int dx, int dy, int dz)
{
    ...
    for (i = 0; (i < partpoint->numpix) && (!blocked_by); i++) {
        ...
        if ((Cement.val[getInt3dindex(Cement,xp,yp,zp)] > POROSITY) && (!blocked_by)) {
    ...
    ***/
    ...
    if ((alumdo==1)&&((Volf[K2SO4] + Volf[NA2SO4]) > 0.0)) {
        ...
        ***/
        ...
        volin = (Volf[C3A] + Volf[C4AF]) / (Volf[C3A] + Volf[C4AF] + Volf[K2SO4] + Volf[NA2SO4]);
        ...
        if (volin < 1.0 && volin > 0.0) {
            ...
            ***/
            ...
            stat3d();
            rdesire = ((Surff[C3A] + Surff[C4AF]) / (Surff[C3A] + Surff[C4AF] + Surff[K2SO4] + Surff[NA2SO4]))
            ...
            ***/
            ...
            if (rdesire > 0.0) {
                ...
                ***/
                    ...
                    rdesire = ((Surff[K2SO4] + Surff[NA2SO4])/(Surff[C3A]+Surff[C4AF] + Surff[K2SO4] + Surff[NA2SO4]))
                        * (float)(Surface[alumval]+Surface[K2SO4]);
...
int surfpix(int xin, int yin, int zin)
{
    ...
    ***/
    ...
    if (Cemreal.val[getInt3dindex(Cemreal,ix1,yin,zin)] == POROSITY) npix++;
    ...
    if (Cemreal.val[getInt3dindex(Cemreal,xin,iy1,zin)] == POROSITY) npix++;
    ...
    if (Cemreal.val[getInt3dindex(Cemreal,xin,yin,iz1)] == POROSITY) npix++;
                ...
                if (valin != POROSITY && valin != EMPTYP
                    ...
                    ***/
                    ...
                    for (k = 1; k <= 6; k++) {
                        ...
                        if (Cemreal.val[getInt3dindex(Cemreal,ix1,iy1,iz1)] == POROSITY) {
                            Surface[valin]++;
                        }
                    }
                }
                        ...
                        for (j = 0; j < Ysyssize; j++) {
                            for (i = 0; i < Xsyssize; i++) {
                                if (Cemreal.val[getInt3dindex(Cemreal,i,j,k)] == ASG) {
                                    ...
                                    totfapix++;
                                    tot[AMSIL]++;
                                }  else if (Cemreal.val[getInt3dindex(Cemreal,i,j,k)] == ANHYDRITE) {
                                    totfapix++;
                                    tot[ANHYDRITE]++;
                                }
                            }
                     ...
                     ***/
                     ...
                     switch (dim) {
                        ...
                        case 2:           /* Z-direction flight */
                            ...
                            while ((newsite != iz) && ((Cemreal.val[getInt3dindex(Cemreal,ix,iy,newsite)] == POROSITY)
    ...
    if (fadchoice == 0) {
        ...
        ***/
        ...
        for (iz = 0; iz < Zsyssize; iz++) {
            for (iy = 0; iy < Ysyssize; iy++) {
                for (ix = 0; ix < Xsyssize; ix++) {
                    if (Cemreal.val[getInt3dindex(Cemreal,ix,iy,iz)] == FLYASH) totcnt++;
                }
            }
        }
    }
