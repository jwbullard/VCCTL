...
int breakflocs(FILE *pfile, short int *p, short int *part,
	short int *in, short int *jn, short int *kn, int xsize, int ysize, int zsize,
    ...
    for (k = 0; k < pz; k++) {
      ...
      for (j = 0; j < py; j++) {
        ...
        for (i = 0; i < px; i++) {
          ...
          part[m] = inval;
          if ((npartmin > part[m]) && (part[m] != 0)) npartmin = part[m];
          if ((npartmax < part[m]) && (part[m] != 0)) npartmax = part[m];
        }
      }
    }
    ...
    for (k = 0; k < zsize; k++) {
      ...
      for (j = 0; j < ysize; j++) {
        ...
        for (i = 0; i < xsize; i++) {
          ...
          oinval = part[m];
          inval = p[m];
          if (inval == C3S || inval == C2S || inval == C3A || inval == OC3A
            || inval == C4AF || inval == GYPSUM 
            || inval == HEMIHYD || inval == ANHYDRITE
            || inval == CACO3 || inval == NA2SO4 || inval == K2SO4) {
        ...
        for (i = 0; i < xsize; i++) {
          ...
          if (inval == C3S || inval == C2S || inval == C3A || inval == OC3A
            ...
            for (ijk = 0; ijk < 27 && !done; ijk++) {
              ...
              if (ipp == C3S || ipp == C2S || ipp == C3A || ipp == OC3A
                || ipp == C4AF || ipp == GYPSUM
                || ipp == HEMIHYD || ipp == ANHYDRITE
                || ipp == CACO3 || ipp == NA2SO4 || ipp == K2SO4) {
