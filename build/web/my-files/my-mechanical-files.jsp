<%-- 
    Document   : my-mechanical-files
    Created on : Oct 28, 2011, 2:11:06 PM
    Author     : bullard
--%>
...
<h3>My Mechanical Properties Files</h3>
<html:form action="/my-files/editMyMechanicalFiles.do">
	<bean:size id="elasticModuliOperationsNumber" name="myMechanicalFilesForm" property="elasticModuliOperations" />
	<logic:greaterThan name="elasticModuliOperationsNumber" value="0">
		<fieldset id="elastic-moduli-operations">
			...
			<nested:iterate name="myMechanicalFilesForm" property="elasticModuliOperations" indexId="ind">
						...
						<div class="collapsable-content expanded">
								...
								<nested:notEmpty property="filesList">
									<nested:iterate property="filesList" indexId="i">
										...
										<tr class="odd-file">
											...
											<td>
												<a href="javascript:viewMechanicalFile('elasticModuliOperations(<%=operationName%>).filesList[<%=i%>]','show-elastic-moduli-file-content_<%=ind%>_<%=i%>','to-reload');" id="show-elastic-moduli-file-content_<%=ind%>_<%=i%>">Show</a>
												<nested:link action="/my-files/downloadMechanicalFile.do" property="linkParameters">Export</nested:link>
												...
												<nested:equal property="viewContent" value="true">
													...
													<nested:equal property="type" value="image">
														...
														<div id="elastic-moduli-file-content_<%=ind%>_<%=i%>" class="to-reload file-image"><nested:link action="/my-files/viewImage.do" target="_blank" property="linkParameters"><img src="/vcctl/image/<%=sliceName%>" /></nested:link></div>
													</nested:equal>
												</nested:equal>
							...
							<nested:notEmpty property="filesList">
								...
								<logic:greaterThan name="filesNumber" value="0">
									Export all files for this operation in one zip file: <nested:link action="/my-files/downloadMechanicalFile.do" paramId="operation" paramProperty="name" styleClass="link"><nested:write property="zipArchiveName" /></nested:link>
								</logic:greaterThan>
							</nested:notEmpty>
						</div>
					</fieldset>
