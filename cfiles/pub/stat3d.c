...
int main(void)
{
			...
			for (ix = 0; ix < xsyssize; ix++) {
				...
				if (valin < NPHASES) {
					...
					***/
					...
					if (valin == OC3A) volume[C3A]++;

					if (valin != POROSITY && valin != DRIEDP
						&& valin != EMPTYDP && valin != EMPTYP) {
					...
					***/
					...
					if (valin == OC3A) mass[C3A] += ((double)Specgrav[valin]);
	...
	***/
	...
	fprintf(statfile,"Phase\tVol.Pix\tSurf.Pix\tVol.frac\tSurf.frac\tMass.frac\n");
