...
long int Volpartmin[NUMSOURCES][MAXSIZECLASSES],Volpartmax[NUMSOURCES][MAXSIZECLASSES];
...
void genparticles(int type, int numsources, long int vol[NUMSOURCES][MAXSIZECLASSES],
    float sizeeachmin[NUMSOURCES][MAXSIZECLASSES], float sizeeachmax[NUMSOURCES][MAXSIZECLASSES], FILE *fpout);
...
int checkpart(int xin, int yin, int zin, int nxp, int nyp, int nzp,
    int volume, int phase2, int wflg);
...
int getsystemsize(void)
{
    ...
    ***/
    ...
    Agg = ibox((long)Xsyssize,(long)Ysyssize,(long)Zsyssize);
    if (!Agg) {
        return(MEMERR);
    }
                ...
                for (k = zin - radd; ((k <= zin + radd) && (!nofits)); k++) {
                    ...
                    ***/
                    ...
                    if ((dist - 0.5) <= (float)radd) {
                        ...
                        if ((Agg[xp][yp][zp] != POROSITY) && (Agg[xp][yp][zp] != ITZ)) nofits = 1;
                    }
    ...
    if (diff > 0) {
        ...
        for (i = 1; i <= nxp; i++) {
            for (j = 1; j <= nyp; j++) {
                for (k = 1; k <= nzp; k++) {
                    if (Bbox[i][j][k] == AGG &&
                        ( (Bbox[i+1][j][k] == POROSITY) || (Bbox[i-1][j][k] == POROSITY)
                          || (Bbox[i][j+1][k] == POROSITY) || (Bbox[i][j-1][k] == POROSITY)
                          || (Bbox[i][j][k+1] == POROSITY) || (Bbox[i][j][k-1] == POROSITY)
                        ) ) {
                        /* add i,j,k to surface pixels */
...
*    additz
*
*     For real shape particles, adds a layer of id ITZ
*     around the periphery of the particle.
*
...
void additz(int nxp, int nyp, int nzp)
{
    ...
    for (k = 1; k < nzp; k++) {
        for (j = 1; j < nyp; j++) {
            for (i = 1; i < nxp; i++) {
                if (Bbox[i][j][k] == AGG) {
                    if (Bbox[i+1][j][k] == POROSITY) Bbox[i+1][j][k] = ITZ;
                    if (Bbox[i-1][j][k] == POROSITY) Bbox[i-1][j][k] = ITZ;
                    if (Bbox[i][j+1][k] == POROSITY) Bbox[i][j+1][k] = ITZ;
                    if (Bbox[i][j-1][k] == POROSITY) Bbox[i][j-1][k] = ITZ;
                    if (Bbox[i][j][k+1] == POROSITY) Bbox[i][j][k+1] = ITZ;
                    if (Bbox[i][j][k-1] == POROSITY) Bbox[i][j][k-1] = ITZ;
                    if (Bbox[i+1][j+1][k] == POROSITY) Bbox[i+1][j+1][k] = ITZ;
                    if (Bbox[i+1][j-1][k] == POROSITY) Bbox[i+1][j-1][k] = ITZ;
                    if (Bbox[i-1][j+1][k] == POROSITY) Bbox[i-1][j+1][k] = ITZ;
                    if (Bbox[i-1][j-1][k] == POROSITY) Bbox[i-1][j-1][k] = ITZ;
                    if (Bbox[i+1][j][k+1] == POROSITY) Bbox[i+1][j][k+1] = ITZ;
                    if (Bbox[i+1][j][k-1] == POROSITY) Bbox[i+1][j][k-1] = ITZ;
                    if (Bbox[i-1][j][k+1] == POROSITY) Bbox[i-1][j][k+1] = ITZ;
                    if (Bbox[i-1][j][k-1] == POROSITY) Bbox[i-1][j][k-1] = ITZ;
                    if (Bbox[i][j+1][k+1] == POROSITY) Bbox[i][j+1][k+1] = ITZ;
                    if (Bbox[i][j+1][k-1] == POROSITY) Bbox[i][j+1][k-1] = ITZ;
                    if (Bbox[i][j-1][k+1] == POROSITY) Bbox[i][j-1][k+1] = ITZ;
                    if (Bbox[i][j-1][k-1] == POROSITY) Bbox[i][j-1][k-1] = ITZ;
                    if (Bbox[i+1][j+1][k+1] == POROSITY) Bbox[i+1][j+1][k+1] = ITZ;
                    if (Bbox[i+1][j+1][k-1] == POROSITY) Bbox[i+1][j+1][k-1] = ITZ;
                    if (Bbox[i+1][j-1][k+1] == POROSITY) Bbox[i+1][j-1][k+1] = ITZ;
                    if (Bbox[i+1][j-1][k-1] == POROSITY) Bbox[i+1][j-1][k-1] = ITZ;
                    if (Bbox[i+1][j+1][k+1] == POROSITY) Bbox[i-1][j+1][k+1] = ITZ;
...
*        long int pvol holds the volume in voxels of each size class
...
*    Called by:    create
***/
void genparticles(int type, int numsources, long int pvol[NUMSOURCES][MAXSIZECLASSES],
    float minsize[NUMSOURCES][MAXSIZECLASSES], float maxsize[NUMSOURCES][MAXSIZECLASSES], FILE *fpout)
{
    ...
    int klow,khigh,mp,newshapefile,numlines,begin,end;
    int oldabsdiff,absdiff,pixfrac,cx,cy,cz;
    int typeeach[NUMAGGBINS],ival,phaseid;
    long int numpartplaced,vol,volmin,volmax,volcrit,diff,ntotal;
    long int voleach[NUMAGGBINS],vpmin[NUMAGGBINS],vpmax[NUMAGGBINS],lval;
    ...
    float length,width,volfractoplace;
    ...
    ***/
    ...
    if (type == (int)COARSE) strcpy(typestring,"coarse");
    if (type == (int)FINE) strcpy(typestring,"fine");
    if (Shape == SPHERES) strcpy(shapestring,"spherical");
    if (Shape != SPHERES) strcpy(shapestring,"real-shape");

    if (Verbose) {
        printf("\nPlacing %s %s aggregate particles now...",shapestring,typestring);
        fflush(stdout);
    }
    ...
    switch (Shape) {
        ...
        case SPHERES:
            ...
            for (ig = 0; ig < numgen; ig++) {
                ...
                if ((2.0 * fdmin) > (float)(Size_safety_coeff * Mindimen)) {
                    ...
                    volmin = vpmin[ig];
                    volmax = vpmax[ig];
                    if (volmax <= volmin) volmax = volmin + 1;
                    critdiam = (float)(Size_safety_coeff * Mindimen);
                    fdmax = critdiam/2.0;
                    volcrit = diam2vol(critdiam);
                    if (volcrit > volmax) volcrit = volmax;
                    if (volcrit < volmin) volcrit = volmin;
                    volfractoplace = (float)(((float)(volcrit - volmin))/((float)(volmax - volmin)));
                    ntotal = (long int)((1.0 - volfractoplace) * voleach[ig]);
                }

                if ((2.0 * fdmax) < (Resolution_safety_coeff * Resolution)) {
                    /* entire class is too small for system */
                    ntotal = voleach[ig];
                } else if ((2.0 * fdmin) < (float)(Resolution_safety_coeff * Resolution)) {
                    ...
                    if (volcrit < volmin) volcrit = volmin;
                    if (volcrit > volmax) volcrit = volmax;
                    volfractoplace = (float)(((float)(volmax - volcrit))/((float)(volmax - volmin)));
                    ntotal += (long int)((1.0 - volfractoplace) * voleach[ig]);
                }
                    ...
                    do {
                        ...
                        ***/
                        ...
                        if (tries > MAXTRIES) {
                            ...
                            printf("\nTotal volume desired in this bin was %ld",voleach[ig]);
                            printf("\nActual volume _placed  in this bin was %ld",ntotal);
        ...
        default:
            ...
            for (ig = 0; ig < numgen; ig++) {
                ...
                numpershape = max(((int)(((float)(voleach[ig]-ntotal)/(float)volmin) / (float)Shapesperbin)),1);

                numpartplaced = 0;

                while (ntotal < voleach[ig]) {
                    if (Verbose) {
                        printf("\n\n**Placed %d of %d total pixels in size range %d",(int)ntotal,(int)voleach[ig],ig);
                        fflush(stdout);
                    }
                    ...
                    if (Verbose) {
                        printf("\nEntering do loop, vol = %ld pixels...",vol);
                        fflush(stdout);
                    }
                    do {
                        if (vol > 19) {
                            newshapefile = 0;
                            if (ntotal == 0 || (!(numpartplaced%numpershape)) || toobig || !foundpart) {
                                ...
                                ***/
                                ...
                                sprintf(filename,"%s%s%c%s",Pathroot,Shapeset,Filesep,line[n1].name);
                                ...
                                ***/
                                ...
                                if (Verbose) {
                                    ...
                                    printf("volmin = %ld volmax = %ld vol = %ld pixels",volmin,volmax,vol);
                                    printf("\nTabulated volume from anm file before scaling = %f pixels",volume);
                                    printf("\nCalculated volume from SH coefficients before scaling = %f pixels",volumecalc);
                            ...
                            ***/
                            ...
                            do {
                                if (na == 0) {
                                    ...
                                    */
                                    ratio[na] = ratio[na-1] * pow((((float)vol)/(float)pcount[na]),(1./3.));
                                    for (n = 0; n <= Nnn; n++) {
                                        for (m = n; m >= -n; m--) {
                                ...
                                */
                                ...
                                if ((nxp < (int)(0.8 * Xsyssize))
                                    ...
                                    if (partc == 0) {
                                        ...
                                        if (Verbose) {
                                            printf("\nAfter image function, nominal particle volume %d, actual %d pixels",(int)vol,(int)partc);
                                            fflush(stdout);
                                        }
                                        toobig = 0;
                                        foundpart = 1;
                            ...
                            } while(labs(partc - vol) > max(4,pixfrac) && na < 3 && !toobig);

                            if (Verbose) {
                                printf("\nConverged? partc = %d and vol = %ld, na = %d",partc,vol,na);
                                fflush(stdout);
                            }
                    ...
                    do {
                        ...
                        ***/
                        ...
                        nofit = checkpart(x,y,z,nnxp,nnyp,nnzp,vol,0,Check);
                    ...
                    ***/
                    ...
                    nump = checkpart(x,y,z,nnxp,nnyp,nnzp,vol,phaseid,Place);
                    ntotal += nump;
                    N_total += nump;
                    numpartplaced++;
                    if (Verbose) printf("\n\t\t\tntotal = %ld out of %ld",ntotal,voleach[ig]);
                    if (Verbose) printf("\n\t\t\tN_total = %ld N_target = %ld",N_total,N_target);
...
void create(int type, int numtimes)
{
    ...
    int num_sources,ns;
    long int vol[NUMSOURCES][MAXSIZECLASSES],inval1;
    int ip,diam,radmin[NUMSOURCES][MAXSIZECLASSES],radmax[NUMSOURCES][MAXSIZECLASSES];
    ...
    printf("\nHow many %s aggregate sources (1 - %d)? ",typestring,(int)NUMSOURCES);
    read_string(buff,sizeof(buff));
    ...
    if (Shape != SPHERES) {
        ...
        ***/
        ...
        if (!Bbox) Bbox = ibox((long)(Xsyssize),(long)(Ysyssize),(long)(Zsyssize));
    ...
    for (ns = 0; ns < num_sources; ns++) {
        printf("Source %d:  Take %s aggregate shapes from what data set?",ns+1,typestring);
            ...
            ***/
            ...
            for(ip = 0; ip < numsize[ns]; ip++) {
                ...
                if ((2.0 * rvalmax) < (Resolution_safety_coeff * Resolution)) {
                    ...
                    /*
                    freeallmem();
                    bailout("genaggpack","Entire size class too fine for system resolution");
                    exit(1);
                    */
                }
...
void measure(void)
{
    ...
    fprintf(outfile,"\nPhase counts are: \n");
    fprintf(outfile,"\tPorosity = %ld \n",npor);
    fprintf(outfile,"\tAggregate = %ld \n",nagg);
    fprintf(outfile,"\tITZ = %ld \n",nitz);
    fclose(outfile);
    ...
    ***/
    ...
    for (i = 0; i < Xsyssize; i++) {
        for (j = 0; j < Ysyssize; j++) {
            ...
            if (((Agg[i][j][k] == npix)
                    && ((Agg[i][j][Zsyssize-1] == npix)
                    || (Agg[i][j][Zsyssize-1] == (npix + Burnt))))
                || ((Agg[i][j][Zsyssize-1] > 0)
                    && (Agg[i][j][k] > 0)
                    && (Agg[i][j][k] < Burnt)
                    && (npix == AGG || npix == ITZ))) {
...
void outmic(void)
{
    int ix,iy,iz,valout;
    int transparent;
    char filen[MAXSTRING],buff[MAXSTRING],filestruct[MAXSTRING],ch;
    FILE *outfile,*infile;
...
void freeallmem(void)
{
    ...
    if (Agg) free_ibox(Agg,Xsyssize,Ysyssize);
    if (Bbox) free_ibox(Bbox,Xsyssize,Ysyssize);
