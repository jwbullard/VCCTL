...
int main(void)
{
    ...
    float res;
    double **ndeep;
    char filein[MAXSTRING],fileout[MAXSTRING],buff[MAXSTRING],instring[MAXSTRING];
        ...
        ***/
        ...
        for (j = 0; j < dytot; j++) {
            for (i = 0; i < dxtot; i++) {
                ...
                fprintf(outfile,"%d ",(int)((ndeep[i][j]*green[image[i][j]])+0.5));
