...
int parthyd(void)
{ 
	long int *norig,*nleft,mult1,mult2;
	...
	norig = (long int *)calloc((size_t)NPARTHYD,sizeof(long int));
	if (!norig) {
		printf("\n\nCould not allocate space for norig array.");
		return(MEMERR);
	}

	nleft = (long int *)calloc((size_t)NPARTHYD,sizeof(long int));
	if (!nleft) {
	...
	***/
	...
	if (Verbose) printf("NPARTHYD is %d",NPARTHYD);
	for (ix = 0; ix < Xsyssize; ix++) {
		if (Verbose) printf("\tx = %d\n",ix);
		for (iy = 0; iy < Ysyssize; iy++) {
			for (iz = 0; iz < Zsyssize; iz++) {
				if (Micpart[ix][iy][iz] != 0) {
					...
					valmic = Mic[ix][iy][iz];
					if ( (valmic == C3S) || (valmic == C2S)
						|| (valmic == C3A) || (valmic == C4AF) || (valmic == OC3A)
						|| (valmic == K2SO4) || (valmic == NA2SO4) ) {

						if (valpart >= mult1 * NPARTHYD) {
		                                        if (Verbose) printf("\t\tReallocating nleft now... ");
							mult1++;
							nleft = (long int *)realloc((long int *)nleft,(size_t)(mult1*NPARTHYD));
		                                        if (Verbose) printf("... Done!\n");
							if (!nleft) {
								printf("\n\nERROR in parthyd:  Could not reallocate space for nleft array.");
								fflush(stdout);
								free(norig);
								return(MEMERR);
							}
					...
					if ( (valmicorig == C3S) || (valmicorig == C2S)
						|| (valmicorig == C3A) || (valmicorig == C4AF) || (valmicorig == OC3A)
						|| (valmicorig == K2SO4) || (valmicorig == NA2SO4) ) {

						if (valpart >= mult2 * NPARTHYD) {
		                                        if (Verbose) printf("\t\tReallocating norig now... ");
							mult2++;
							norig = (long int *)realloc((long int *)norig,(size_t)(mult2*NPARTHYD));
		                                        if (Verbose) printf("... Done!\n");
							if (!norig) {
								printf("\n\nERROR in parthyd:  Could not reallocate space for norig array.");
								fflush(stdout);
								free(nleft);
								return(MEMERR);
							}
