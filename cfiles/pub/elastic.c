...
{
	if (u) free_drect(u,Syspix);
	if (gb) free_drect(gb,Syspix);
	if (b) free_drect(b,Syspix);
	if (h) free_drect(h,Syspix);
	if (ib) free_irect(ib,Syspix);
	if (pix) free_sivector(pix);
	if (part) free_sivector(part);
	if (Aa) free_dbox(Aa,Xsyssize,36);
	if (A) free_drect(A,36);
	if (Vv) free_drect(Vv,Xsyssize);
	if (A1) free_dbox(A1,Xsyssize,36);
	if (Cc) free_drect(Cc,6);
	if (K) free_dvector(K);
	if (G) free_dvector(G);
...
void ppixel(int nphase, int *doitz, int *nagg1)
{
      ...
      FILE *infile,*pinfile;
      char filein[MAXSTRING],pfilein[MAXSTRING],buff[MAXSTRING],instring[MAXSTRING];
      FILE *fpout;
	...
	read_string(filein,sizeof(filein));
	  ...
	  printf("Enter whether to break connections between\n");
		...
		*doitz = atoi(buff);
        ...
        printf("\n%s\n",Outfolder);
        printf("Enter fully resolved name of file to output calculated effective moduli: ");
...
/*  reading it in from a file, this should be done inside this subroutine. */
	...
	if (Sever) {
		...
		if (breakflocs(pinfile,pix,part,in,jjn,kn,Xsyssize,Ysyssize,Zsyssize,Version,Res)) {
			fclose(pinfile);
			exit(1);
		}
		fclose(pinfile);
      ...
      int nphase;
{
      ...
      float x,y,z;
      int is[8],m,l,k,j,i,ijk,n1,n2,n3,n,mm,nn,ii,jj,ll,kk,i3,i8,m8,m4,nxy;
      long int m3;
...
void stress(int nx, int ny, int nz, long int ns, int doitz, int micro, int ilast)
{
...
void modlayer(int *nagg1)
{
	...
	***/
	...
	for (j = 0; j < 36; j++) {	/* 36 components in FULL stress and strain tensors */
        for (i = 0; i < 36; i++) {
            A[i][j] = 0.0;
	...
	***/
	...
	for (ii = 0; ii < Xsyssize; ii++) {
		...
		***/
		...
		c12avg = (Cc[0][1] + Cc[0][2] + Cc[1][0] + Cc[1][2]
					+ Cc[2][0] + Cc[2][1]) / 6.0;
...
int main(void){
 	int m3,i,j,k,n,nx,ny,nz,nphase,ijk,nxy,i1,j1,k1,npoints,kmax,ldemb;
...
/*  C3S (Voigt bounds on C3S, i.e. locking grain, as recommended by Angel by literature search,
    especially calculations by Manzano et al in 2009 */
/*
        phasemod[C3S][0]=112.0;
        phasemod[C3S][1]=52.0;
*/
/*  C2S (Voigt bounds on beta-C2S, i.e. locking grain, as recommended by Angel by literature search,
    especially calculations by Manzano et al in 2009 */
/*
        phasemod[C2S][0]=111.0;
        phasemod[C2S][1]=53.1;
*/
/* C3A (DFT simulations by Manzano et al 2009, C3A is cubic so Voigt and Reuss bulk moduli are equal, we use
       the Voigt value for shear modulus because of the assumption of locked grain boundaries) */
/*
        phasemod[C3A][0]=102.9;
        phasemod[C3A][1]=54.4;
*/
/* C4AF (Voigt estimate on brownmillerite, i.e. locking grain, as recommended by Angel by literature search,
   especially high-pressure diffraction studies of Ross et al. 2002 and Vanpeteghem et al 2008 */
/*
...
*/
/* Gypsum (Voigt values determined from ultrasonic interferometry, by Haussuehl, 1965).
         May want to use Reuss values for those voxels at the surface of the particle */
...
/* Anhydrite (from compression tests on single crystals, Schewerdtner et al 1965.
    All tensor components measured to within 7% error.  Voigt and Reuss values calculated from these */
...
/* Hemihydrate (i.e. bassanite).  Reuss values reported in a powder compression study.
   Angel suggests estimating Voigt values based on the anisotropy ratio of the crystal,
   which is 1.30 */
...
*/
/* inert  --- assume same as calcite mineral.  Data from Brillouin spectroscopy (Chen et al 2001)
      provides Voigt values on bulk and shear moduli */
/*
...
*/
/* CaCO3 (calcite form, Voigt values determined by Brillouin spectroscopy, Chen et al. 2001) */
/*
...
*/
			...
			*/
			...
			if(gg>gtest){
				...
				printf("%lf %lf %lf %lf %lf %lf\n",strxxt/(double)ns,stryyt/(double)ns,strzzt/(double)ns,strxzt/(double)ns,stryzt/(double)ns,strxyt/(double)ns);
				printf("strains:  xx,yy,zz,xz,yz,xy\n");
				printf("%lf %lf %lf %lf %lf %lf \n",sxxt/(double)ns,syyt/(double)ns,szzt/(double)ns,sxzt/(double)ns,syzt/(double)ns,sxyt/(double)ns);
				fflush(stdout);
			}
		}
		...
		printf("stresses:  xx,yy,zz,xz,yz,xy\n");
		printf("%lf %lf %lf %lf %lf %lf\n",strxxt,stryyt,strzzt,strxzt,stryzt,strxyt);
		printf("strains:  xx,yy,zz,xz,yz,xy\n");
	...
	if (npoints == 1) {
		...
		***/
		...
		for (i = 0; i < NSP; i++) {
			if (prob[i] > pthresh) {
				stressall[i][12] = (stressall[i][0] + stressall[i][1]
					+ stressall[i][2])/(exx + eyy + ezz);
				stressall[i][12] /= 3.0;
			
				stressall[i][13] = (stressall[i][3]/exy);
				stressall[i][13] += (stressall[i][4]/exz);
				stressall[i][13] += (stressall[i][5]/eyz);
				stressall[i][13] /= 6.0;	/* Divide by extra 2.0 because global shear strains are doubled */

				stressall[i][14] = (9.0 * stressall[i][12]
					* stressall[i][13])/((3.0 * stressall[i][12])
					+ stressall[i][13]);

				stressall[i][15] = ((3.0 * stressall[i][12])
					- (2.0 * stressall[i][13]))/
...
{ 
    ...
    double coarse_agg_vf[NUMCOARSESOURCES],eitz,itzwidth,poissitz,kitz,gitz,tdval,val;
    ...
    if (num_fine_sources > (int)NUMFINESOURCES) num_fine_sources = (int)NUMFINESOURCES;
    */
    ...
    for (m = 0; m < (int)NUMFINESOURCES; m++) {
        ...
        if (val > 0) {
          ...
          printf("\nEnter BULK modulus for fine aggregate %d (in GPa): ",num_fine_sources+1);
          read_string(buff,sizeof(buff));
          kfine = atof(buff);
          printf("%f",kfine);
          printf("\nEnter SHEAR modulus for fine aggregate %d (in GPa): ",num_fine_sources+1);
          read_string(buff,sizeof(buff));
          gfine = atof(buff);
          printf("%f",gfine);
              ...
              for (i = 0; i < 3; i++) {
                  ...
                  if (!feof(gfile)) {
                      ...
                      if (i == 1) {
                          ...
                          Vf_concelas[N_concelas] = fine_agg_vf[num_fine_sources] * (atof(buff));
                          ...
                          printf("\n%d: Diam = %f, Vf = %f, sum = %f",N_concelas,Diam_concelas[N_concelas],Vf_concelas[N_concelas],sum);
                          N_concelas++;
                          cnt++;
                      }
                  }
    ...
    if (num_coarse_sources > (int)NUMCOARSESOURCES) num_coarse_sources = (int)NUMCOARSESOURCES;
    */

    for (m = 0; m < (int)NUMCOARSESOURCES; m++) {
        printf("\n\nEnter volume fraction of coarse aggregate %d: ",num_coarse_sources+1);
        read_string(buff,sizeof(buff));
        val = atof(buff);
        if (val > 0) {
            ...
            printf("\nEnter BULK modulus for coarse aggregate %d (in GPa): ",num_coarse_sources+1);
            read_string(buff,sizeof(buff));
            kcoarse = atof(buff);
            printf("%f",kcoarse);
            printf("\nEnter SHEAR modulus for coarse aggregate %d (in GPa): ",num_coarse_sources+1);
            read_string(buff,sizeof(buff));
            gcoarse = atof(buff);
            printf("%f",gcoarse);
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    if (!feof(gfile)) {
                        ...
                        if (i == 1) {
                            ...
                            Vf_concelas[N_concelas] = coarse_agg_vf[num_coarse_sources] * (atof(buff));
                            sum += Vf_concelas[N_concelas];
    ...
    /* Bubble sort on each aggregate type individually, then assigne average diameters */
    ...
    fprintf(fpout,"\tMortar_Cylinder_Compressive_strength (power fit): %.4f MPa\n",cylinder_strngth);
    ...
    fprintf(fpout,"\tConcrete_Cylinder_Compressive_strength (0.62*cube): %.4f MPa\n",concrete_cube_strngth*0.624);
    fclose(fpout);
    return(0);
}
...
void effective(double itzwidth, double kitz, double gitz)
{
    ...
    for (i = 0; i <= N_concelas; i++) {
        ...
        printf("\nK_concelas[%d] = %f, nui = %f, nuitz = %f",i,K_concelas[i],nui,nuitz);

        geff = G_concelas[i] / gitz - 1.0;
        printf(", geff[%d] = %f",i,geff);

        eta1 = geff * (7.0 - 10.0 * nuitz)
                    * (7.0 + 5.0 * nui) + 105.0 * (nui - nuitz);
        ...
        aa -= (2.0 * (63.0 * geff * eta2 + 2.0 * eta1 * eta3)
                   * pow(c,(7.0/3.0)));

        aa += (252.0 * geff * eta2 * pow(c,(5.0/3.0)));

        aa -= (50.0 * geff * (7.0 - 12.0 * nuitz
               + 8.0 * nuitz * nuitz ) * eta2 * c);

        aa += (4.0 * (7.0 - 10.0 * nuitz) * eta2 * eta3);

        bb = (-2.0 * geff * (1.0 - 5.0 * nuitz) * eta1 * pow(c,(10.0/3.0)));

        bb += (2.0 * (63.0 * geff * eta2 + 2.0 * eta1 * eta3)
                   * pow(c,(7.0/3.0)));
        ...
        cc = 4.0 * geff * (5.0 * nuitz - 7.0) * eta1
                 * pow(c,(10.0/3.0));

        cc -= (2.0 * (63.0 * geff * eta2 + 2.0 * eta1 * eta3)
                   * pow(c,(7.0/3.0)));
...
void slope(double *kk, double *gg, double k, double g)
{
    ...
    *gg = 0.0;
    ...
    for (i = 0; i <= N_concelas; i++) {
        *kk += (Vf_concelas[i] * ((k + (q * g)) * (K_concelas[i]/k - 1.0) / (K_concelas[i] + (q * g))));
        *gg += (Vf_concelas[i] * (5.0 * (k + (q * g)) * (G_concelas[i] - g)
