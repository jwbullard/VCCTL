...
#define MOLEFACTOR 1.0E12       /* cubic micrometers per
									cubic centimeter */
...
int main(void)
{
	...
	***/
	...
	if (read_imgheader(micfile,&Version,&Xsyssize,&Ysyssize,&Zsyssize,&Res)) {
		fclose(micfile);
	...
	for (ix = 0; ix < Xsyssize; ix++) {
		for (iy = 0; iy < Ysyssize; iy++) {
			for (iz = 1; iz < Zsyssize + 1; iz++) {
				...
				Mic[ix][iy][iz] = inval;
				React[ix][iy][iz] = 0;
				Density[iz] += ((Specgrav[inval] / MOLEFACTOR)
									/ Layer_volume);
				...
				if ((inval == POROSITY) || (inval == EMPTYP)
					|| (inval == DIFFCSH) || (inval == DIFFCH)
					|| (inval == DIFFGYP) || (inval == DIFFC3A)
					|| (inval == DIFFC4A) || (inval == DIFFFH3)
					|| (inval == DIFFETTR) || (inval == DIFFCACO3)
					|| (inval == DIFFAS) || (inval == DIFFANH)
					|| (inval == DIFFHEM) || (inval == DIFFCAS2)
					|| (inval == DIFFCACL2) || (inval == DRIEDP)
					|| (inval == EMPTYDP) || (inval == MS)) {
				...
				if (inval == FRIEDEL) Friedelorig[iz]++;
				if (inval == GYPSUM) Gypsumorig[iz]++;
				if (inval == AFMC) Afmcorig[iz]++;
				if (inval == CACO3) Ccorig[iz]++;
			}
		}
	}
	...
	***/
	...
	for (iz = 1 ; iz <= initdepth; iz++) {
		...
		nadd = numadd - (long)Ndiff[iz]; /* number ants left to add
											to this layer */
		while (nadd > 0 && nleft > 0) {
			...
			***/
			...
			ich = 0;
			if ((phid == POROSITY) || (phid == CSH)
				|| (phid == EMPTYP) || (phid == DRIEDP)
	...
	for (icyc = 1; icyc <= ncyc; icyc++) {
		...
		if (nadd > 0) {
			...
			for (ia = 0; ia < nadd; ia++) {
				...
				prand = ran1(Seed);
				iy = (int)((float)Ysyssize * prand);
				if (iy >= Ysyssize) iy = Ysyssize - 1;
				if (ix >= Xsyssize) ix = Xsyssize - 1;
		...
		for (iant = 1; iant <= norg; iant++) {
			...
			***/
			...
			if (ich != 0) {
				...
				if ((phid != POROSITY) && (phid != AFMC)
					&& (phid != CSH) && (phid != ETTR)
					&& (phid != AFM) && (phid != ETTRC4AF)
					&& (phid != EMPTYP) && (phid != DRIEDP)
					&& (phid != C3AH6) && (phid != POZZCSH)
					&& (phid != SLAGCSH)) {
						...
						if (React [cxn][cyn][czn] == SPERAFM) {
							...
							***/
                        	...
                        	if (ran1(Seed) < ptest) {
								...
								Friedelcount[czn]++;	
								Density[czn] += ((Specgrav[FRIEDEL]
									- Specgrav[AFM])/MOLEFACTOR)/Layer_volume;
							} else {
								Mic[cxn][cyn][czn] = ETTR;
								Ettrorig[czn]++;
								Density[czn] += ((Specgrav[ETTR]
									- Specgrav[AFM])/MOLEFACTOR)/Layer_volume;	
							}
					...
					if ((ran1(Seed) < preact)
						...
						if (React[cxn][cyn][czn] == SPERC3AH6) {
							...
							Friedelcount[czn]++;
							Density[czn] += ((Specgrav[FRIEDEL]
								- Specgrav[C3AH6])/MOLEFACTOR)/Layer_volume;
					...
					if ((ran1(Seed) < preact)
						...
						if (React[cxn][cyn][czn] == SPERAFMC) {
							...
							Friedelcount[czn]++;
							Density[czn] += ((Specgrav[FRIEDEL]
								- Specgrav[AFMC])/MOLEFACTOR)/Layer_volume;
					...
					&& (Nrafm[czn] > (Afmorig[czn] * 0.9))
					&& (Nrafmc[czn] > (Afmcorig[czn] * 0.9))) {

					ich = 0;
					if ((ran1(Seed) < preact)
						&& (React[cxn][cyn][czn] < SPERETTR)) {
						...
						if (React[cxn][cyn][czn] == SPERETTR) {
							...
							***/
							...
							if (ran1(Seed) < ptest) {
								...
								Gypsumcount[czn]++;
								Density[czn] += ((Specgrav[GYPSUM]
									- Specgrav[ETTR])/MOLEFACTOR)/Layer_volume;
				...
				} else if ((phid == ETTRC4AF)
					&& (Nrafm[czn] > (Afmorig[czn] * 0.9))) {

					ich = 0;
					if ((ran1(Seed) < preact)
						&& (React[cxn][cyn][czn] < SPERETTRC4AF)) {
				...
				} else if (((phid == CSH) || (phid == POZZCSH)
					|| (phid == SLAGCSH)) && (!sel1)) {

					Cfre = Ndiff[czn] * 1000.0 * MASSCACL2 * 2.0 
						* MOLEFACTOR/(MwCaCl2 * (((double)Nrcap[czn])+Nrgel[czn]));
...
void extphase(int phtomake, int xcur, int ycur, int zcur)
{
    ...
    if ((phtomake == FRIEDEL) && (found)) {
		Friedelcount[ztry]++;
		Nrcap[ztry]--;	
		Density[ztry] += (((Specgrav[FRIEDEL]-Specgrav[AFM])/MOLEFACTOR)
							/ Layer_volume);
    }
    ...
    if ((phtomake == ETTR) && (found)) {
		...
		if (React[xcur][ycur][zcur] == SPERAFM) {
			Density[ztry] += (((Specgrav[ETTR]-Specgrav[AFM])/MOLEFACTOR)
							/ Layer_volume);
		} else {
			Density[ztry] += (((Specgrav[ETTR]-Specgrav[C3AH6])/MOLEFACTOR)
							/ Layer_volume);
		}
	...
	if ((phtomake == CACO3) && (found)) {
		Ccorig[ztry]++;
		Nrcap[ztry]--;
		Density[ztry] += (((Specgrav[CACO3]-Specgrav[AFMC])/MOLEFACTOR)
							/ Layer_volume);
...
void allmem(void)
{
	...
	if (!Znew || !Ynew || !Xnew || !Clchemisorb || !Clreacted || !Density
			...
			|| !Strainbrucite || !Straingyp || !Nrgel || !Nrcap
			|| !Ccorig || !Afmcorig || !Nrafmc || !Ettrc4aforig
			|| !C3ah6orig || !Gypsumorig || !Ettrorig || !Afmorig
			|| !Nrc3ah6 || !Nrafm || !Nrettrc4af || !Nrettr
			|| !Ndiff || !React || !Mic) {
...
void freeallmem(void)
{
	if (Mic) free_sibox(Mic,Xsyssize+2,Ysyssize+2);
	if (React) free_sibox(React,Xsyssize+2,Ysyssize+2);
	if (Ndiff) free_ivector(Ndiff);
	if (Nrettr) free_ivector(Nrettr);
	if (Nrafm) free_ivector(Nrafm);
	if (Nrc3ah6) free_ivector(Nrc3ah6);
	if (Afmorig) free_ivector(Afmorig);
	if (Ettrorig) free_ivector(Ettrorig);
	if (Gypsumorig) free_ivector(Gypsumorig);
	if (C3ah6orig) free_ivector(C3ah6orig);
	if (Ettrc4aforig) free_ivector(Ettrc4aforig);
	if (Nrafmc) free_ivector(Nrafmc);
	if (Afmcorig) free_ivector(Afmcorig);
	if (Ccorig) free_ivector(Ccorig);
	if (Nrcap) free_livector(Nrcap);
	if (Nrgel) free_dvector(Nrgel);
	if (Straingyp) free_ivector(Straingyp);
	if (Strainbrucite) free_ivector(Strainbrucite);
	if (Strainettr) free_ivector(Strainettr);
	if (Strainfriedel) free_ivector(Strainfriedel);
	if (Gypsumcount) free_ivector(Gypsumcount);
	if (Friedelcount) free_ivector(Friedelcount);
	if (Density) free_dvector(Density);
	if (Clreacted) free_dvector(Clreacted);
	if (Clchemisorb) free_dvector(Clchemisorb);
	if (Xnew) free_sivector(Xnew);
	if (Ynew) free_sivector(Ynew);
	if (Znew) free_sivector(Znew);
