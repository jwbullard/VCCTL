...
{ 
    ...
    for (k = 0; k < num_fine_sources; k++) {
        ...
        if (fine_agg_vf[k] > 0.0) {
            ...
            printf("\nEnter fully-resolved name of grading file for fine aggregate %d: ",k+1);
    ...
    for (k = 0; k < num_coarse_sources; k++) {
        ...
        if (coarse_agg_vf[k] > 0.0) {
            ...
            printf("\nopening diameter (mm) and one for fraction retained.");
            printf("\nThe columns must be TAB-DELIMITED.");
            printf("\n\nEnter fully-resolved name of grading file for coarse aggregate %d: ",k+1);
                ...
                for (i = 0; i < 3; i++) {
                    ...
                    if (!feof(gfile)) {
                        ...
                        if (i == 1) {
                            ...
                            Vf_conctransport[N_conctransport] = coarse_agg_vf[k] * (atof(buff));
                            sum += Vf_conctransport[N_conctransport];
    ...
    /* Bubble sort on each aggregate type individually, then assign average diameters */
    for (k = 0; k < num_fine_sources; k++) {
    ...
    if (fabs(sum - 1.0) > 0.005) {
        ...
        for (i = 0; i < N_conctransport; i++) {
            if (sum > 0.0) Vf_conctransport[i] /= sum;
        }
    }
...
void effective(double itzwidth, double sitz)
{
    ...
    for (i = 0; i < N_conctransport; i++) {
        ...
        printf("\n\tDiam[%d] = %f, itzwidth = %f",i,Diam_conctransport[i],itzwidth);
