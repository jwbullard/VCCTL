...
int main(void)
{
	...
	int valin,ovalin,garb;
        float x_frac_connected,y_frac_connected,z_frac_connected,ave_frac_connected;
	    ...
	    ***/
	    ...
	    for (iz = 0; iz < Zsyssize; iz++) {
		    for (iy = 0; iy < Ysyssize; iy++) {
				...
				Mic[ix][iy][iz] = valin;
                                if (valin == POROSITY || valin == EMPTYP) Npores++;
			}
		    }
	    }
	    ...
	    fclose(micfile);
            ...
            Tot_porosity = ((float)(Npores)) / ((float)(Xsyssize) * (float)(Ysyssize) * (float)(Zsyssize));
            printf("total porosity = %f\n",Tot_porosity);

	    garb = burn3d(EMPTYP,1,0,0);
	    if (garb == MEMERR) {
    		bailout("perc3d-leach","Could not allocate memory in burn3d function (x)");
    		exit(1);
            }
	    ...
	    if (garb == MEMERR) {
		    bailout("perc3d-leach","Could not allocate memory in burn3d function (y)");
		    exit(1);
	    }
	    ...
	    if (garb == MEMERR) {
	    	bailout("perc3d-leach","Could not allocate memory in burn3d function (z)");
		exit(1);
            }
            z_frac_connected = (float)(garb)/(float)(Npores);
            printf("Fraction connected in z direction = %f\n",z_frac_connected);
     
            ave_frac_connected = (x_frac_connected + y_frac_connected + z_frac_connected) / 3.0;
