...
void dissolve(int cycle)
{
    ...
    ***/
    ...
    if (Verbose) {
        ...
        printf("\n    fact = %f",fact);
        printf("\n        Count[CSH] = %ld Tfractw04 = %f Cshscale = %f",Count[CSH],Tfractw04,Cshscale);
                            ...
                            if (pconvert < PCSH2CSH) {
                                ...
                                ***/
                                ...
                                if (calcy > 1.0) {
                                    calcz = calcy - 1.0;
                                    calcy = 1.0;
                                    if (Verbose) printf("WARNING:  Problem of not creating enough pozzolanic CSH during CSH conversion \nCurrent binder temperature is %f C\n",Temp_cur_b);
                                }
...
void freeallmem(void)
{
    ...
    if (Startflag) free_ivector(Startflag,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed ivector Startflag");
    if (Stopflag) free_ivector(Stopflag,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed ivector Stopflag");
    if (Deactphaselist) free_ivector(Deactphaselist,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed ivector Deactphaselist");
    if (Deactfrac) free_fvector(Deactfrac,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Deactfrac");
    if (Reactfrac) free_fvector(Reactfrac,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Reactfrac");
    if (Deactinit) free_fvector(Deactinit,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Deactinit");
    if (Deactends) free_fvector(Deactends,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Deactends");
    if (Deactterm) free_fvector(Deactterm,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Deactterm");
    if (Molarvcsh) free_fvector(Molarvcsh,Ncyc);
    if (Verbose) printf("\nFreed fvector Molarvcsh");
    if (Watercsh) free_fvector(Watercsh,Ncyc);
    if (Verbose) printf("\nFreed fvector Watercsh");
    if (Disprob) free_fvector(Disprob,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Disprob");
    if (Disbase) free_fvector(Disbase,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Disbase");
    if (Discoeff) free_fvector(Discoeff,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Discoeff");
    if (Soluble) free_ivector(Soluble,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed ivector Soluble");
    if (Creates) free_ivector(Creates,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed ivector Creates");
    if (Onepixelbias) free_fvector(Onepixelbias,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector Onepixelbias");
    if (PHsulfcoeff) free_fvector(PHsulfcoeff,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector PHsulfcoeff");
    if (PHfactor) free_fvector(PHfactor,(long int)(NSPHASES + 1));
    if (Verbose) printf("\nFreed fvector PHfactor");
    if (CustomImageTime) free_fvector(CustomImageTime,Tcustomoutputentries);
    if (Verbose) printf("\nFreed fvector CustomImageTime");
    if (DataTime) free_fvector(DataTime,NDataLines);
    if (Verbose) printf("\nFreed fvector DataTime");
    if (DataValue) free_fvector(DataValue,NDataLines);
    if (Verbose) printf("\nFreed fvector DataValue");
    if (TimeHistory) free_fvector(TimeHistory,Ncyc);
