...
int burnset(int d1, int d2, int d3)
{
	...
	***/
    ...
    if (!xformMicpart) {
		...
		printf("\n\tExiting now.");
        fflush(stdout);
        if (xformMic) free_ibox(xformMic,dimensions[0],dimensions[1]);
		return(MEMERR);
	}
    ...
    if (!newmat) {
		...
		printf("\n\tExiting now.");
        fflush(stdout);
        if (xformMicpart) free_ibox(xformMicpart,dimensions[0],dimensions[1]);
        if (xformMic) free_ibox(xformMic,dimensions[0],dimensions[1]);
		return(MEMERR);
	}
	...
	if (!nmatx) {
        ...
        fflush(stdout);
        if (xformMicpart) free_ibox(xformMicpart,dimensions[0],dimensions[1]);
		if (xformMic != NULL) free_ibox(xformMic,dimensions[0],dimensions[1]);
		return(MEMERR);
	}
	if (!nmaty) {
		printf("\nERROR in burnset:");
	...
	***/
							...
							***/
							...
							if ((x1 >= 0) && (x1 < dimensions[0])) {
								...
								***/
								...
								(xformMicpart[qx][qy][qz] == xformMicpart[px][py][pz])
	...
	count_solid = Count[C3S] + Count[C2S] + Count[C3A] + Count[K2SO4] + Count[NA2SO4];
	count_solid += Count[C4AF] + Count[ETTR] + Count[CSH] + Count[POZZCSH] + Count[SLAGCSH];
	count_solid += Count[C3AH6] + Count[ETTRC4AF] + Count[SFUME];
	count_solid += Count[AMSIL] + Count[ASG] + Count[SLAG] + Count[CAS2];

    if (Verbose) printf("Count solids = %ld\n",count_solid);
	if (count_solid > 0) {
		Con_fracs[dir] = (float)nthrough / (float)count_solid;
        if (Verbose) printf("Con_fracs[%d] = %f\n",dir,Con_fracs[dir]);
	}

	tvar = Time_cur + (2.0 * (float)(Cyccnt) - 1.0) * (Beta / Krate);

	if (Con_fracs[dir] > 0.985) setyet = 1;
	...
	***/
	...
	free_ivector(nnewy);
	free_ivector(nnewz);
	if (newmat != NULL) free_ibox(newmat,dimensions[0],dimensions[1]);
	if (xformMicpart != NULL) free_ibox(xformMicpart,dimensions[0],dimensions[1]);
	if (xformMic != NULL) free_ibox(xformMic,dimensions[0],dimensions[1]);
