...
*	References:
*
*		K.A. Snyder, Feng, Keen, and T.O. Mason, Cement and Concrete Research
*			
...
void pHpred(void)
{
	...
	*/
	...
	if (Time_cur > 1.0) {
		...
		***/
		...
		Releasedk += (2.0 * (Flyashmass/Cemmasswgyp)*( Rsfapotassium
							+ (Totfapotassium - Rsfapotassium) * Alpha_fa_cur ));	/* from fly ash */
	    Releasedk /= MMK2O;
		...
		***/
		...
		Releasedna += (2.0 * (Flyashmass/Cemmasswgyp)*( Rsfasodium
	...
	****/
	...
	Concnaplus = (Releasedna)
					/ (volpore + BNa * Alpha_cur + BprimeNa * pozzreact);
		...
		***/
		...
		if (((Concnaplus + Conckplus) > 0.0) && (!Soluble[ETTR])) {
			...
			while ( ((abs(Istrength - Inew) / Istrength) > 0.10) && nt < 10000) {
				...
				Bnow = activeB0 * sqrt(295.0)
						/ (sqrt(Temp_cur + 273.15));
				...
				ActivityCa += (0.2 - (0.0000417 * Istrength))
					* Anow * zCa * zCa * Istrength
					/ sqrt(1000.0);
				...
				ActivityK += (0.2 - 0.0000417 * Istrength)
					* Anow * zK * zK * Istrength
					/ sqrt(1000.0);
				...
				ActivitySO4 += (0.2 - 0.0000417 * Istrength)
					* Anow * zSO4 * zSO4 * Istrength
					/ sqrt(1000.0);
				...
				***/
				...
				for (j = 1; j <= 4; j++) {
					...
					if (Verbose) printf("pH root %d is (%f,%f)\n",j,roots[j].r,roots[j].i);

					if (((roots[j].i) == 0.0) && ((roots[j].r) > 0.0)) {
	
					/* conctest is the computed OH- concentration */

						conctest = sqrt(KspCH / (roots[j].r
							* ActivityCa * ActivityOH * ActivityOH));

						concsulfate1 = KspGypsum / (roots[j].r
							* ActivityCa * ActivitySO4);
				...
				Inew *= 1000.0;
                       ...
                       if (nt >= 10000) {
                           printf("\npHpred was caught in an infinite loop with insoluble ettringite.");
                           fflush(stdout);
                       }
			...
			while (((abs(Istrength - Inew) / Istrength) > 0.10) && nt < 10000) {
				...
				***/
				...
				Conccaplus = (KspCH / (ActivityCa * ActivityOH
					* ActivityOH * Concohminus * Concohminus));
		...
		if (syn_old != 2) {
			...
			test_precip *= Concsulfate * Concsulfate
								* ActivitySO4 * ActivitySO4;

			if (test_precip > KspSyngenite) {

				if (Verbose) printf("Syngenite precipitating at cycle %d\n",Cyccnt);
	...
	*	Reference: K.A. Snyder, Feng, Keen, and Mason, Cement and Concrete Research, 2003 
	*
	...
	***/
	...
	Conductivity += zCa * Conccaplus
			* (lambdaCa_0 / (1.0 + GCa * sqrt(Istrength)));

	Conductivity += zOH * Concohminus
			* (lambdaOH_0 / (1.0 + GOH * sqrt(Istrength)));

	Conductivity += zNa * Concnaplus
			* (lambdaNa_0 / (1.0 + GNa * sqrt(Istrength)));

	Conductivity += zK * Conckplus
			* (lambdaK_0 / (1.0 + GK * sqrt(Istrength)));

	Conductivity += zSO4 * Concsulfate
			* (lambdaSO4_0 / (1.0 + GSO4 * sqrt(Istrength)));
