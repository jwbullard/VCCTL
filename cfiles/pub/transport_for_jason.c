...
double *Xg,*Wg;
double S_conctransport[MAXSIZECLASSES][MAXAGGTYPES];
double Si_conctransport[MAXSIZECLASSES][MAXAGGTYPES];
double Diam_conctransport[MAXSIZECLASSES][MAXAGGTYPES];
double Vf_conctransport[MAXSIZECLASSES][MAXAGGTYPES];
int N_conctransport;
int Ng;

/* const double Conductive_fine_agg_fraction = 0.2782; */
const double Conductive_fine_agg_fraction = 0.0;
const double Conductive_coarse_agg_fraction = 0.0;
...
*/
    ...
    */
    ...
    /* Manually assign relative conductivities for phases */
      ...
      sigma[(EMPTYP)+1][1] = 1.0;
	  sigma[(EMPTYP)+1][2] = 1.0;
	  sigma[(EMPTYP)+1][3] = 1.0;

      sigma[(DRIEDP)+1][1] = 1.0;
	  sigma[(DRIEDP)+1][2] = 1.0;
	  sigma[(DRIEDP)+1][3] = 1.0;

      sigma[(EMPTYDP)+1][1] = 1.0;
	  sigma[(EMPTYDP)+1][2] = 1.0;
	  sigma[(EMPTYDP)+1][3] = 1.0;
...
{ 
    register int i,j,ii,k;
    int itzpix,numfinebins,done;
    double scem,fine_agg_vf,coarse_agg_vf;
    double sitz,condrat,itzwidth[2],tdval;
    ...
    double sum,sum0,sum1,sfine,scoarse,sign;
    double conductive_aggfrac,normal_aggfrac,aggfrac,airfrac;
    ...
    double conductive_fine_agg_vf,normal_fine_agg_vf;
    double conductive_coarse_agg_vf,normal_coarse_agg_vf;
    FILE *gfile,*cempsd;
    ...
    for (j = 0; j < MAXAGGTYPES; j++) {
        for (i = 0; i < MAXSIZECLASSES; i++) {
            S_conctransport[i][j] = 0.0;
            Si_conctransport[i][j] = 0.0;
            Diam_conctransport[i][j] = 0.0;
            Vf_conctransport[i][j] = 0.0;
        }
    }
    ...
    if (!cempsd) {
        ...
        itzwidth[0] = mediansize(cempsd);
        fclose(cempsd);
    }

    if (nagg1 > 0) {  /*  Aggregate must be present for this to execute */

        /* Convert itzwidth[0] to cement paste image pixel widths */
        itzpix = (int)((itzwidth[0]/Res) + 0.5);
    ...
    /* THIS IS WHERE WE DIVIDE THE FINE AGGREGATE INTO CONDUCTIVE
     * AND NON-CONDUCTIVE PORTIONS
     */

    conductive_fine_agg_vf = Conductive_fine_agg_fraction * fine_agg_vf;
    normal_fine_agg_vf = (1.0 - Conductive_fine_agg_fraction) * fine_agg_vf;
    ...
    if (fine_agg_vf > 0.0) {
        ...
        printf("\nEnter conductivity for conductive part of fine aggregate: ");
        read_string(buff,sizeof(buff));
        sfine = atof(buff);
        printf("%f",sfine);
        ...
        while (!feof(gfile)) {
            for (i = 0; i < 3; i++) {
                ...
                if (!feof(gfile)) {
                    ...
                    buff[j] = '\0';
                    if (i == 1) {
                        Diam_conctransport[N_conctransport][0] = atof(buff);
                        Diam_conctransport[N_conctransport][1] = atof(buff);
                    } else if (i == 2) {
                        Vf_conctransport[N_conctransport][0] = conductive_fine_agg_vf * (atof(buff));
                        Vf_conctransport[N_conctransport][1] = normal_fine_agg_vf * (atof(buff));
                        sum0 += Vf_conctransport[N_conctransport][0];
                        sum1 += Vf_conctransport[N_conctransport][1];
                        S_conctransport[N_conctransport][0] = sfine;
                        S_conctransport[N_conctransport][1] = 0.0;
                        Si_conctransport[N_conctransport][0] = sfine;
                        Si_conctransport[N_conctransport][1] = 0.0;
                        printf("\n%d: Conductive: Diam = %f, Vf = %f, sum = %f",N_conctransport,Diam_conctransport[N_conctransport][0],Vf_conctransport[N_conctransport][0],sum0);
                        printf("\n%d: Normal: Diam = %f, Vf = %f, sum = %f",N_conctransport,Diam_conctransport[N_conctransport][1],Vf_conctransport[N_conctransport][1],sum1);
                        N_conctransport++;
                    }
                }
            }
    ...
    conductive_coarse_agg_vf = Conductive_coarse_agg_fraction * coarse_agg_vf;
    normal_coarse_agg_vf = coarse_agg_vf - conductive_coarse_agg_vf;
            ...
            for (i = 0; i < 3; i++) {
                ...
                if (!feof(gfile)) {
                    ...
                    if (i == 1) {
                        ...
                        Vf_conctransport[N_conctransport][0] = conductive_coarse_agg_vf * (atof(buff));
                        Vf_conctransport[N_conctransport][1] = normal_coarse_agg_vf * (atof(buff));
                        sum0 += Vf_conctransport[N_conctransport][0];
                        sum1 += Vf_conctransport[N_conctransport][1];
                        S_conctransport[N_conctransport][0] = scoarse;
                        S_conctransport[N_conctransport][1] = 0.0;
                        Si_conctransport[N_conctransport][0] = scoarse;
                        Si_conctransport[N_conctransport][1] = 0.0;
                        printf("\n%d: Conductive Diam = %f, Vf = %f, sum = %f",N_conctransport,Diam_conctransport[N_conctransport][0],Vf_conctransport[N_conctransport][0],sum0);
                        printf("\n%d: Normal Diam = %f, Vf = %f, sum = %f",N_conctransport,Diam_conctransport[N_conctransport][1],Vf_conctransport[N_conctransport][1],sum1);
                        N_conctransport++;
    ...
    for (ii = 0; ii < 2; ii++) {
        for (i = 0; i < N_conctransport - 1; i++) {
            for (j = (i + 1); j < N_conctransport; j++) {
                if (Diam_conctransport[i][ii] < Diam_conctransport[j][ii]) {
                    tdval = Diam_conctransport[i][ii];
                    Diam_conctransport[i][ii] = Diam_conctransport[j][ii];
                    Diam_conctransport[j][ii] = tdval;
                    tdval = Vf_conctransport[i][ii];
                    Vf_conctransport[i][ii] = Vf_conctransport[j][ii];
                    Vf_conctransport[j][ii] = tdval;
                    tdval = S_conctransport[i][ii];
                    S_conctransport[i][ii] = S_conctransport[j][ii];
                    S_conctransport[j][ii] = tdval;
                    tdval = Si_conctransport[i][ii];
                    Si_conctransport[i][ii] = Si_conctransport[j][ii];
                    Si_conctransport[j][ii] = tdval;
                }
            }
        }
    }
    ...
    for (ii = 0; ii < 2; ii++) {
        for (i = 1; i < (N_conctransport - numfinebins); i++) {
            Diam_conctransport[i][ii] = 0.5 * (Diam_conctransport[i][ii] + Diam_conctransport[i-1][ii]);
        }
    }
    /* Approximation for largest coarse sieve size */
    Diam_conctransport[0][0] *= 1.10;
    Diam_conctransport[0][1] *= 1.10;
    ...
    Diam_conctransport[N_conctransport - numfinebins + 1][0] *= 1.10;
    Diam_conctransport[N_conctransport - numfinebins + 1][1] *= 1.10;
    ...
    if (fabs(sum - 1.0) > 0.005) {
        ...
        for (i = 0; i < N_conctransport; i++) {
            if ((sum0 + sum1) > 0.0) {
                Vf_conctransport[i][0] /= (sum0 + sum1);
                Vf_conctransport[i][1] /= (sum0 + sum1);
            }
        }
    }
    ...
    for (i = 0; i < N_conctransport; i++) {
        printf("\nConductive Diam = %f Vf = %f",Diam_conctransport[i][0],Vf_conctransport[i][0]);
        printf("\nNormal Diam = %f Vf = %f",Diam_conctransport[i][1],Vf_conctransport[i][1]);
    }
    printf("\n");

    conductive_aggfrac = conductive_fine_agg_vf + conductive_coarse_agg_vf;
    normal_aggfrac = normal_fine_agg_vf + normal_coarse_agg_vf;
    aggfrac = conductive_aggfrac + normal_aggfrac;
    printf("\nTotal conductive aggregate volume fraction = %f",conductive_aggfrac);
    printf("\nTotal normal volume fraction = %f",normal_aggfrac);
    ...
    for (i = 0; i < N_conctransport; i++) {
        Vf_conctransport[i][0] *= (conductive_aggfrac / (conductive_aggfrac + normal_aggfrac + airfrac));
        Vf_conctransport[i][1] *= (normal_aggfrac / (conductive_aggfrac + normal_aggfrac + airfrac));
    }
    Diam_conctransport[N_conctransport][1] = 0.04;
    Diam_conctransport[N_conctransport][0] = 0.04;
    S_conctransport[N_conctransport][0] = 0.0;
    S_conctransport[N_conctransport][1] = 0.0;
    Si_conctransport[N_conctransport][0] = 0.0;
    Si_conctransport[N_conctransport][1] = 0.0;
    Vf_conctransport[N_conctransport][0] = 0.0;
    Vf_conctransport[N_conctransport][1] = airfrac / (conductive_aggfrac + normal_aggfrac + airfrac);
    target_agg_vf = (normal_aggfrac + conductive_aggfrac + airfrac);

    itzwidth[1] = itzwidth[0];
    itzwidth[0] = 0.0;
    ...
    while ((i < EMT_ITERATIONS) && (!done)) {
        ...
        */
        ...
        for (j = 0; j < Ng; j++) {
            ...
            /*
            printf("\n\t\tGaussian weights X[%d] W[%d] = %f,%f, Quadrature point = %f",j,j,Xg[j],Wg[j],conductivity);
            */
            ...
            for (k = 0; k < 2; k++) {
                for (ii = 0; ii <= N_conctransport; ii++) {
                    if (Diam_conctransport[ii][k] > 0.0) {
                        ba = (Diam_conctransport[ii][k] + 2.0 * itzwidth[k])/Diam_conctransport[ii][k];
                        alpha = pow(ba,3.0);
                        ss += ( Vf_conctransport[ii][k] * (3.0 * alpha * (S_conctransport[ii][k] - conductivity))
                                     / ((2.0 * conductivity) + S_conctransport[ii][k])
                           );
                    }
                    /*
                    printf("\n\t\t\tii = %d, k = %d, Vf particle = %f, S particle = %f, ss = %f",ii,k,Vf_conctransport[ii][k],S_conctransport[ii][k],ss);
                    */
                }
            }
        ...
        printf("\n\tVf = %f (target = %f), ssave = %f, slope = %f",xtrack[i],target_agg_vf,ssave[i],ss);
        fflush(stdout);
...
void effective(double *itzwidth, double sitz)
{
    ...
    for (ii = 0; ii < 2; ii++) {
        for (i = 0; i < N_conctransport; i++) {
            ...
            printf("\n\tDiam[%d][%d] = %f, itzwidth[%d] = %f",i,ii,Diam_conctransport[i][ii],ii,itzwidth[ii]);
            ba = (Diam_conctransport[i][ii] + 2.0 * itzwidth[ii])/Diam_conctransport[i][ii];
            c = pow(ba,3.0);

            printf("\nba = %f and c = %f",ba,c);
            printf("\nSi_conctransport[%d][%d] = %f and sitz = %f",i,ii,Si_conctransport[i][ii],sitz);
            S_conctransport[i][ii] = sitz * (
                                (2.0 * (Si_conctransport[i][ii] - sitz))
                                + (c * (Si_conctransport[i][ii] + (2.0 * sitz)))
                             );
            S_conctransport[i][ii] /= ( (c * (Si_conctransport[i][ii] + 2.0 * sitz)) - (Si_conctransport[i][ii] - sitz) );

            printf("\nS_conctransport[%d][%d] = %f",i,ii,S_conctransport[i][ii]);
    ...
    *ss = 0.0;
    ...
    for (ii = 0; ii < 2; ii++) {
        for (i = 0; i <= N_conctransport; i++) {
            ba = (Diam_conctransport[i][ii] + 2.0 * itzwidth[ii])/Diam_conctransport[i][ii];
            alpha = pow(ba,3.0);
            *ss += ( Vf_conctransport[i][ii] * (3.0 * alpha * (S_conctransport[i][ii] - s))
                                     / ((2.0 * s) + S_conctransport[i][ii])
               );
        }
    }
