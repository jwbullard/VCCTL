...
from .base_service import ServiceError, NotFoundError, AlreadyExistsError
...
class OperationService:
    ...
    def create_operation(self, name: str, operation_type: str, notes: str = None) -> Operation:
        ...
        try:
            ...
            if existing:
                raise AlreadyExistsError(f"Operation '{name}' already exists")
        except NotFoundError:
            pass  # This is expected
        
        # Create operation using new model
        operation = Operation(
            name=name,
        ...
        # Save to database (simplified - no need for mark_queued since we set status in constructor)
